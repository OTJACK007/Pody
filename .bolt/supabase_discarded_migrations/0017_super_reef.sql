...
-- Create feature status enum if it doesn't exist
DO $$ 
BEGIN
...
DROP POLICY IF EXISTS "Authenticated users can suggest features" ON features;
DROP POLICY IF EXISTS "Users can vote on features" ON feature_votes;
DROP POLICY IF EXISTS "Users can change their votes" ON feature_votes;
...
DROP TRIGGER IF EXISTS set_timestamp ON features;
