...
-- Create improved export function with proper error handling
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE v.id = video_id 
    AND (v.status = 'public' OR v.publisher_id = auth.uid())
  ) THEN
  ...
  IF 'ai_summary' = ANY(sections) THEN
    ...
    SELECT 
      ...
      video_id,
      v.title,
      COALESCE(a.key_takeaways[1], 'No summary available'), -- Handle null case
      'video',
      v.title,
      jsonb_build_object(
        'readingTime', '5 min',
